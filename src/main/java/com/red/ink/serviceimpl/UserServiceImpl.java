			...
			if (forgetPassword.getUsername() != null && forgetPassword.getUsername() != "") {
				if (forgetPassword.getUsername().equals(loggedInUser.get().getUsername())) {
					if (forgetPassword.getEmail().equals(loggedInUser.get().getEmail())) {
							...
							if (forgetPassword.getEmail() != null && forgetPassword.getEmail() != "") {
								...
								if (forgetPassword.getEmail().equals(loggedInUser.get().getEmail())) {
									...
									SendMail.sendMail(receivers, "Hi " + loggedInUser.get().getName() + ","
											...
											+ loggedInUser.get().getUsername() + "<br />" + "Password: " + password
											+ "<br /><br />" + "Thanks & Regards," + "<br />" + "REDINK Sports Team",
											"REDINK Reset Password", null);

									userRepository.save(loggedInUser.get());

									return jsonConstactor.responseCreation(true, "password can be send your mail", null,
											null, null);
								} else {
									return jsonConstactor.responseCreation(false, "Email Id Cannot Matched", null, null,
											null);
								}
							} else {
						...
						return jsonConstactor.responseCreation(false, "Failed", "emaild id can't be Matched!!!", null,
								null);
					}
...
//							forgot.setOtpVerificationTime(new Date());
						...
						if (forgotPassword.getEmail().equals(loggedInUser.get().getEmail())) {
							String emailLoginContent = "Website: " + "<a href = " + Constants.URL
									+ ">www.redinksports.com</a><br/>";
							String[] receivers = new String[] { forgotPassword.getEmail() };
							SendMail.sendMail(receivers, "Hi " + loggedInUser.get().getName() + "," + "<br /><br />"
									+ "Your REDINK forgotpassword OTP is successful done. Please find your login details below."
									+ "<br />" + emailLoginContent + "Username: " + loggedInUser.get().getUsername()
									+ "<br />" + "OTP: " + otp + "<br /><br />" + "Thanks & Regards," + "<br />"
									+ "REDINK Sports Club", "REDINK ForgotPassword OTP", null);
							// loggedInUser.get().setOtp(otp);
							return jsonConstactor.responseCreation(true, "OTP can be send your mail", null, null, null);
						}

					

				}
					return jsonConstactor.responseCreation(false, "Failed", "Emai-Id id can't be Matched!!!", null,
							null);
			}
		...
		if (changePassword != null) {
			...
			if (user.isPresent()) {
				...
				if (forgotpasswordOTP != null) {
					...
					if (otpmatches) {
						if(forgotpasswordOTP.getOtp().equals(changePassword.getOtp())) {
							if (changePassword.getNewPassword() != null && changePassword.getConfirmPassword() != null) {
								...
								return jsonConstactor.responseCreation(false, "Password Miss match", "check the password",
										null, null);

							}
							return jsonConstactor.responseCreation(false, "Check Password Fields", "Try Again", null, null);
						}
						return jsonConstactor.responseCreation(false, "OTP Miss Match", "OTP Miss Match", null, null);
					}
					return jsonConstactor.responseCreation(false, "The OTP is Expired", "The OTP is Expired", null, null);
				} else {
					return jsonConstactor.responseCreation(false, "No value present","No value present", null, null);
				}
			} else {
				return jsonConstactor.responseCreation(false, "No value present", "No value present", null, null);
			}
	...
	public ResponseEntity<Object> findAll(String token) {
		...
		try {
			...
			if(loggedInUser.get().getRole().getRole().equals("ADMIN")) {
				...
				usersList =users.stream()
						   .filter(user -> !user.getRole().getRole().equals("ADMIN"))
						   .collect(Collectors.toList());
				
			}else if(loggedInUser.get().getRole().getRole().equals("Trainer")) {
				usersList =users.stream()
						   .filter(user -> user.getRole().getRole().equals("Trainer") || user.getRole().getRole().equals("Player") )
						   .filter(user ->user.getSelectgame().equals(loggedInUser.get().getSelectgame()) ) 
						   .collect(Collectors.toList());
				
			}else if(loggedInUser.get().getRole().getRole().equals("Player")){
				usersList =users.stream()
						   .filter(user -> user.getRole().getRole().equals("Player"))
						   .filter(user ->user.getSelectgame().equals(loggedInUser.get().getSelectgame())) 
						   .collect(Collectors.toList());
...
//					user1.setUploadPAN(user.getUploadPAN());
					...
					if (currentUser.get().getUploadAadhar().equals(user.getUploadAadhar())) {
						...
						if (user.getUploadAadhar() != null && user.getUploadAadhar() != "") {
							...
							if (user.getUploadAadhar() != null && user.getUploadAadhar() != "") {
								String fileName = user.getUploadAadhar()
										.substring(user.getUploadAadhar().lastIndexOf("/"));
								String filePath = Constants.USERAADHARIMG + fileName;
								File dest = new File(filePath);
								dest.delete();
					...
					if (loggedInUser.get().getUsername().equals(otpRequest.getUsername())) {
						if (loggedInUser.get().getWhatsappNo().equals(otpRequest.getPhoneNumber())) {
							if (otpRequest.getPhoneNumber() != null) {
								...
								ForgotpasswordMobileOTP forgotpasswordOTP = forgotpasswordMobileOTPRepository
										.findByUser(loggedInUser.get());
								if (forgotpasswordOTP == null) {
									ForgotpasswordMobileOTP forgot = new ForgotpasswordMobileOTP(otp, new Date(),
											loggedInUser.get());
			...
			if (user.isPresent()) {
				...
				if (forgotpasswordOTP != null) {
					...
					if (otpmatches) {
						...
						return jsonConstactor.responseCreation(false, "The OTP is Expired", "Try Again", null, null);
					}
					return jsonConstactor.responseCreation(false, "The OTP is Expired", "Try Again", null, null);
				} else {
					return jsonConstactor.responseCreation(false, "No value present", null, null, null);
				}
			} else {
				return jsonConstactor.responseCreation(false, "No value present", null, null, null);
			}
...
//											Files.delete(myPath);
					...
					if (user1.getEmail() != null) {
						...
						SendMail.sendMail(receivers, "Hi " + user1.getName() + "," + "<br /><br />"
								+ "Your REDINK  Your Account Create Successfully. Please find your login details below."
								+ "<br />" + emailLoginContent + "Username: " + user1.getUsername() + "<br />"
								+ "Password: " + password + "<br /><br />" + "Thanks & Regards," + "<br />"
								+ "REDINK Sports ", "Team", null);
					}
		...
		return jsonConstactor.responseCreation(false, "User Token Is not valid", null, null, null);
	}
